...
func NewHandler(
	...
	return libhttp.WithErrorFunc(
		func(ctx context.Context, resp http.ResponseWriter, req *http.Request) error {
			...
			glog.V(2).
				Infof("read records from topic %s and partition %d and offset %d with limit %d started", topic, partition.Int32(), offset.Int64(), limit)
			...
			glog.V(2).
				Infof("read %d records from topic %s and partition %d and offset %d with limit %d completed", len(page.Records), topic, partition.Int32(), offset.Int64(), limit)
			return nil
		},
	)
}
